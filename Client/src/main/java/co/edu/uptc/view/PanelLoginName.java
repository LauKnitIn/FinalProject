    ...
    private void addOptions() {// MOD para enviar nombre al cliente
        ...
        continuarButton = addButton("Continuar", 480, 500, e -> {
            ...
            if (!(name.isEmpty())) {
                ...
                boolean isAdmin = ((View) SwingUtilities.getWindowAncestor(PanelLoginName.this)).getIsAdmin();    
                if(isAdmin){
                    showPanelDifficulty();
                }else{
                    //((View) SwingUtilities.getWindowAncestor(PanelLoginName.this)).showPanelChooseWord();;// enviar    
                }    
                                                                // nombre
                showPanelDifficulty();
            }
