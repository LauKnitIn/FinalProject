package co.edu.uptc.view;

import java.awt.BasicStroke;
import java.awt.Color;
import java.awt.Dimension;
import java.awt.Graphics;
import java.awt.Graphics2D;
import java.awt.RenderingHints;

import javax.swing.JPanel;

import co.edu.uptc.view.constants.FontPalette;

public class WordDisplayPanel extends JPanel {

    private String progress = "_ _ _ _ _ _";

    public WordDisplayPanel() {
        setOpaque(false);
    }

    public void setProgress(String progress) {
        this.progress = progress;
        revalidate();// MOD
        repaint();
    }

    /*
     * @Override
     * protected void paintComponent(Graphics g) {
     * super.paintComponent(g);
     * Graphics2D g2 = (Graphics2D) g;
     * 
     * g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING,
     * RenderingHints.VALUE_ANTIALIAS_ON);
     * g2.setColor(Color.BLACK);
     * g2.setFont(FontPalette.WORD_DISPLAY_FONT);
     * 
     * int spacing = 80;
     * int startX = 20;
     * int baseY = 40;
     * 
     * // Divide por espacio para respetar el formato del servidor: "_ _ _ A _ S"
     * String[] chars = progress.split(" ");
     * 
     * for (int i = 0; i < chars.length; i++) {
     * int x = startX + i * spacing;
     * g2.setStroke(new BasicStroke(3));
     * g2.drawLine(x, baseY, x + 50, baseY);
     * 
     * if (!chars[i].equals("_")) {
     * g2.drawString(chars[i], x + 10, baseY - 10);
     * }
     * }
     * 
     * }
     */

    @Override
    public Dimension getPreferredSize() {
        int charsCount = progress.split(" ").length;
        int rows = (charsCount + 5) / 6; // filas necesarias (redondeo hacia arriba)
        int width = 20 + 6 * 80; // startX + 6 letras * spacing
        int height = 40 + rows * 60; // baseY + filas * lineSpacing
        return new Dimension(width, height);
    }

    @Override
    protected void paintComponent(Graphics g) {
        super.paintComponent(g);
        Graphics2D g2 = (Graphics2D) g;

        g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
        g2.setColor(Color.BLACK);
        // Usa tu fuente personalizada o prueba con Arial si tienes problemas
        g2.setFont(FontPalette.WORD_DISPLAY_FONT);

        int spacing = 80;
        int startX = 20;
        int baseY = 40;
        int lineSpacing = 60;

        String[] chars = progress.split(" ");

        for (int i = 0; i < chars.length; i++) {
            int row = i / 6;
            int col = i % 6;

            int x = startX + col * spacing;
            int y = baseY + row * lineSpacing;

            g2.setStroke(new BasicStroke(3));
            g2.drawLine(x, y, x + 50, y);

            if (!chars[i].equals("_")) {
                g2.drawString(chars[i], x + 10, y - 10);
            }
        }

    }

    public void updateProgress(String newProgress) {
    this.progress = newProgress;
    revalidate();  // recalcula tamaño si es necesario
    repaint();     // repinta el panel con la nueva palabra
}

}
